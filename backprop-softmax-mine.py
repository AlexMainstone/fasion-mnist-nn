...
class BackPropagation:
    ...
    def backward(self, y):
        ...
        for l in range(self.L-2, 0, -1):
            ...
            self.delta[l] = np.multiply(np.matmul(np.transpose(self.w[l+1]), self.delta[l+1]), self.phi_d(self.z[l]))
            self.db[l] = self.delta[l]
            # self.dw[l] = np.dot(self.a[l-1], self.delta[l])
            for j in range(len(self.dw[l])):
                for k in range(len(self.dw[l][j])):
                    self.dw[l][j][k] = self.a[l-1][k] * self.delta[l][j]
...
def main():
    ...
    # bp.backward(bp.trainY[0])
    # print(np.argmax(bp.dw[1]))
    bp.sgd()
